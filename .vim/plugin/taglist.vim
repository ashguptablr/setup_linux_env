...
" Copyright: Copyright (C) 2002-2008 Yegappan Lakshmanan
...
let s:tlist_def_javascript_settings =
            \ 'javascript;c:class;m:method;v:global;f:function'
...
let s:tlist_def_sql_settings = 'sql;f:functions;' .
           ...
           \ 'D:domains;x:MLTableScripts;y:MLConnScripts;'.
...
" When closing the taglist window, make sure autocmds are disabled. Otherwise,
" if the user has added an autocmd top open the taglist window again, then
...
function! s:Tlist_Window_Exit_Only_Window()
    ...
    if v:version < 700
	if winbufnr(2) == -1
            call s:Tlist_Exe_Cmd_No_Acmds('bdelete');
	    quit
	endif
    else
	if winbufnr(2) == -1
